"""Shared async startup for the Docker host Device Connect CLI."""
...
def log_run_config(params: DeviceConnectRunParams) -> None:
    ...
    logger.info("=== Device Connect Docker host driver (dcd) ===")
    logger.info(
        "device_id=%s tenant=%s portal=%s simulate=%s docker_host=%s",
        cfg.device_id,
        cfg.tenant,
        cfg.portal,
        cfg.simulate,
        cfg.docker_host or "(default)",
    )
    logger.info(
...
def gather_cli_run_params(args: Namespace) -> DeviceConnectRunParams:
    ...
    config = DriverConfig(
        ...
        simulate=simulate,
        docker_host=args.docker_host or env.docker_host,
        state_poll_hz=args.state_poll_hz or env.state_poll_hz,
...
async def run_device_connect(params: DeviceConnectRunParams) -> None:
    ...
    logger.info("Starting DeviceRuntime (Docker host driver)")
    try:
        await runtime.run()
    except Exception:
